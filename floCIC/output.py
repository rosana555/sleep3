...
def flocic_decompress_to_bgr(payload: bytes, flocic_exe: str) -> np.ndarray:
...
class App:
    ...
    def on_message_frame(self, client, userdata, msg):
        ...
        payload = msg.payload
        try:
            # floCIC header sanity check
            w, h = parse_flocic_header(payload)
            img = flocic_decompress_to_bgr(payload, FLOCIC_EXE)
        except Exception:
            # fallback to normal image decode
            img = cv2.imdecode(np.frombuffer(payload, dtype=np.uint8), cv2.IMREAD_COLOR)
            if img is None:
                print("Frame decode failed (neither floCIC nor image)")
                return
